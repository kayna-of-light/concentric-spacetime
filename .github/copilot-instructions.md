...
|----------|----------|--------|
| **GEO-1** | Laplacian spectrum on nested concentric spheres {S²(P_k)} with covering connections | RESOLVED for mass exponents and selection rules (NB186). S¹ cascade = l=0 sector of S² gradient flow (embedding theorem). Per-l geometric damping κ_eff(l,k) = κ(1 + l(l+1)/P_k²) PRODUCES x_lep = 3 (l=3) and x_q base = 3/2 (l=2). Covering p₂=3 filter PRODUCES singlet/triplet. 200/189 NOT absorbable by per-ℓ damping (NB187: quark C₀ changes only 0.058% from ℓ=0→ℓ=2) — confirmed as S¹ cascade mechanism. C₀ atlas: R₃-level, branch-averaged, κ-dependent (NB185). |
