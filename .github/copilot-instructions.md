...
| Geometric C₀ | NB196 | Floquet degeneracy theorem: linearized cascade has 4-fold degenerate multiplier e^{−κ} (M(t) lower-triangular, constant diagonal −κ). No mode splitting → C₀ = 1 in linear regime. Per-level IC decay hierarchy: γ₀/κ=0.41 > γ₁/κ=0.30 > γ₂/κ=0.11 > γ₃/κ≈0 (inner homogenize faster). Quark/lepton covering split: quark C₀ at R₃ invariant under inner (j₁,j₂) conditioning (CV=0.05%) → effective covering = p₃p₄=35; lepton C₀ requires full P₄=210 (CV=12.4%). Chirality label a₃ from p₂=3 is the symmetry filter. 0 identities. GD-2 still OPEN. |
| Geometric C₀ | NB196 | Floquet degeneracy theorem: linearized cascade has 4-fold degenerate multiplier e^{−κ} (M(t) lower-triangular, constant diagonal −κ). No mode splitting → C₀ = 1 in linear regime. C₀ ≠ 1 requires nonlinear sin() covering potential. Per-level IC decay hierarchy: γ₀/κ=0.41, γ₁/κ=0.30, γ₂/κ=0.11, γ₃/κ≈0. Quark/lepton covering split: quark C₀ at R₃ invariant under inner (j₁,j₂) conditioning (CV=0.05%) → effective covering = p₃p₄=35; lepton C₀ requires full P₄=210 (CV=12.4%). Chirality label a₃ (p₂=3) is the symmetry filter. 0 identities. GD-2 OPEN. |

### Current State (Post-NB196)
...
- **C₀ mechanism** (NB195-196): C₀ = √(p₃p₄) is genuinely dynamical — requires full cascade ODE, κ = 1/√P₄ specifically (extreme κ-sensitivity), 210-branch IC diversity (99.7% variance at g1), and cascade cross-coupling from lower levels. Simple wrapping model fails (43% off). Effective decay γ₃/κ ≈ 0.893. NB196: Floquet degeneracy theorem proves linearized cascade has 4-fold degenerate multiplier e^{-κ} (no mode splitting → C₀ = 1). Nonlinear sin() covering potential breaks the degeneracy. Quark/lepton covering split: quark C₀ determined by 35-group structure (CV=0.05%), lepton requires full 210 (CV=12.4%). Chirality a₃ is the filter. GD-2 (analytic derivation): OPEN.
