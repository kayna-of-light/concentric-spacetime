...
> Last updated after NB137 (The Quark Exponent — quark window-0 exponent investigation; current best descriptor for the measured R₃ quark exponent is 100/63 = (4/7)×(25/9), which fits slightly better than 2^(2/3); per-level survey reveals R₀ quark exponent ≈ 4/7 at 36.8 ppm; exponent remains OPEN mechanistically).
...
| **The Quark Exponent** | NB137 | Quark window-0 exponent investigation; per-level survey; wrapping anatomy; algebraic candidate battery; T-convergence | 0 new identities (investigation). x_eff(s/d) = 1.5866463961 is perfectly T-independent (spread = 0 across T=1000–5000 in the refined notebook check). Initial battery suggested 2^(2/3) as the best simple candidate, but extended notebook analysis found the measured value is described slightly better by 100/63 = (4/7)×(25/9), winning over 2^(2/3) consistently across T=1000,2000,5000. Per-level survey: R₀ quark x = 0.5714495813 ≈ 4/7 (+36.8 ppm). Cross-level ratio x_q(R₃)/x_q(R₀) = 2.776529 is close to 25/9 (−449.5 ppm). Mechanism: j₃ controls 92.7% of the steady-state variance at ci=11, and the j₃=4 branch satisfies s̄(4)/s̄(0) = 2.999539 ≈ p₂, lowering the lattice sum enough to move x from 1.5715 to 1.5866. The exponent remains OPEN mechanistically, but the best current descriptor is composite rather than the earlier pure 2^(2/3) law. |
