...
> Last updated after NB72.

## Summary

| Metric | Value |
|--------|-------|
| **Structural identities** | 133 |
| **Free parameters** | 0 |
...
| **Radial Mass Channel** | NB72 | Three-level mass architecture: R₄ (gen1→2), R₃ (inter-sector), R₂ (gen2→3), exponent hierarchy | 4 identities: exponent hierarchy x₄/x₃=φ(5) (#133), m_c/m_u two-channel (#134), m_b/m_s R₂ channel (#135), m_b/m_d combined (#136); 2 NULLs: top quark scope boundary (#137), lepton hint (#138) |
...
**Scope boundary**: The top quark mass requires physics beyond the multiplicative R-channel architecture. Possible mechanisms: (1) non-multiplicative interaction between R₂ and R₃ at the p=3 and p=5 interference boundary; (2) the level-1 channel (p=2, R₁) entering for the heaviest fermion; (3) a dynamical symmetry-breaking mechanism specific to the third-generation up-type sector.
...
- **Fermion mass ratios** now have a **three-level mass architecture** with zero free parameters: (1) **Level 4** (R₄, generation): m_s/m_d = 19.92 at −0.032σ (NB70); (2) **Level 3** (R₃, inter-sector): m_c/m_u = R₃^{λ(35)/(2π)} × R₄^{φ(210)/(2π)} = 627.4 at +0.76σ (NB72); (3) **Level 2** (R₂, 3rd generation): m_b/m_s = R₂^{φ(30)/(2π)} = 45.83 at +2.4% (NB72). Down-type quarks are FULLY predicted across all 3 generations: m_s/m_d, m_b/m_s, and m_b/m_d. The exponent hierarchy x₄/x₃ = φ(5) = 4 is exact. **Top quark mass remains an open frontier** — the multiplicative three-channel formula fails by 10×, indicating the top quark requires a mechanism beyond R-channel multiplication. Lepton masses show suggestive alignment (R₃(lep)^{x₃} = 16.18 vs m_τ/m_μ = 16.82) but remain inconclusive.
