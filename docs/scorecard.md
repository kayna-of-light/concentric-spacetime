...
| **L-Sector Decoupling** | NB191 | GD-1 completion: Euler-Lagrange of S² action. C_pol matrix computed exactly (sympy-verified). Key result: C_pol(l'>0, 0; m) = 0 for all l'>0 by Legendre orthogonality (∫P_{l'}(x)×1 dx = 0). Holds nonlinearly: constant field composed with ANY map = constant. S¹ cascade = exact l=0 invariant subspace. Even-parity C_pol(m=0) block = [[1, 8/35], [0, -1/7]] — upper triangular, entries ONLY from framework primes {2,3,5,7}. Unidirectional coupling: l=2 → l=0 with strength 8/35 but l=0 → l=2 blocked. C_pol(0,2;0) = 8/35 = φ(P₄)/P₄ = sin²θ_W — Weinberg angle IS the geometric back-reaction coefficient of quark (l=2) sector on cascade (l=0). Odd-parity fractions involve p₅+ primes. C₀ does NOT come from cross-l coupling (closed by C_pol(2,0)=0). | 3 identities (#351: L-sector decoupling theorem — DERIVED, EXACT. #352: even-parity upper triangularity — DERIVED, EXACT. #353: C_pol(0,2) = sin²θ_W = 8/35 — PATTERN-MATCHED → DERIVED by NB192). GD-1: COMPLETE. |
| **Chebyshev Coupling Formulas** | NB192 | Universal analytic formulas for per-prime Chebyshev coupling on S². C_T_p(0,2;0) = (p²−1)/(4p²−1) — l=0↔l=2 coupling for ANY covering degree p (sympy-verified p=2..10). C_T_p(2,2;0) = −15p²/[(4p²−1)(4p²−9)] — l=2 self-coupling for any p≥1 (sympy-verified p=2..8). Three-way Weinberg chain: at p=p₂=3, (p₂²−1)/(4p₂²−1) = 8/35. Numerator: p₂²−1 = 8 = p₁³ (by Catalan/Mihailescu: 3²−2³=1 is the unique consecutive perfect power pair). Denominator: 4p₂²−1 = 35 = p₃p₄. So sin²θ_W = p₁³/(p₃p₄) = φ(P₄)/P₄ — the Chebyshev formula connects to primorial arithmetic via the ONLY pair of consecutive perfect powers. Self-coupling at p₂=3: 4p₂²−9 = 27 = p₂³, so C(2,2) = −1/p₄. Even-parity block: [[1, φ(P₄)/P₄], [0, −1/p₄]], trace = λ(p₄)/p₄ = 6/7, det = −1/p₄. 200/189 classified as algebraic bridge between cascade (4/7×25/9) and S² (3/2×200/189) factorizations — not per-prime geometric. GD-3: ANSWERED. | 5 identities (#354: universal (0,2) formula — DERIVED, EXACT. #355: universal (2,2) formula — DERIVED, EXACT. #356: three-way Weinberg chain via Catalan — DERIVED, EXACT; upgrades #353. #357: C(2,2)=−1/p₄ at p₂=3 — DERIVED, EXACT. #358: even-parity block all framework constants — DERIVED, EXACT). GD-3: ANSWERED. |
| **Chebyshev Coupling Algebra** | NB193 | Extended Chebyshev coupling to odd-parity modes and cross-level structure. New universal formula: C_T_p(1,1;0) = −3/(p²−4) for all odd p (sympy-proved, numerically verified p=3..19). Bilateral spectral democracy: C_T_2(0,l) = (−1)^l/(2l+1) for all l — the p₁=2 covering seeds ALL angular modes with equal structural weight (inverse Legendre norms). Unique to p=2 (sympy-proved l=0..7). Cross-level resonance: C₃(2,2) = C₅(1,1) = −1/p₄, linking quark (l=2) and charge (l=1) sectors through the outermost prime. Forced by three arithmetic identities: 4p₂²−1 = p₃p₄ (twin primes around 2p₂), 4p₂²−9 = p₂³ (unique to p₂=3), p₂p₄ = p₃²−p₁² (difference of squares). All three encode {3,5,7} as the unique arithmetic prime triple with difference 2 (mod 3 proof + exhaustive search to p₂ < 1000). −1/p₄ appears at THREE independent positions: C₂(0,3), C₃(2,2), C₅(1,1). | 4 identities (#359: C(1,1) = −3/(p²−4) — DERIVED, EXACT. #360: C₂(0,l) = (−1)^l/(2l+1) — DERIVED, EXACT. #361: C₃(2,2) = C₅(1,1) = −1/p₄ — DERIVED, EXACT. #362: {3,5,7} unique AP prime triple — DERIVED, EXACT). |
| **Odd-Parity Block** | NB194 | Complete l-parity decomposition: odd-parity block (l=1,3) fully characterized. Bilateral Annihilation Theorem: C_{T_2}(l,l') = 0 for ALL l when l' is odd (T₂(x)=2x²−1 is even, P_{l'}(x) is odd for odd l', integral vanishes). Per-prime odd blocks: p=3 unique with COMPLEX eigenvalues (rotation, |λ|=0.284), p=5,7 real (scaling). Closed-form entries: C(1,3) = −7(p²−1)/[(p²−4)(p²−16)], C(3,1) = −12(p²−1)/[(p²−4)(9p²−4)]. Framework constants: C₃(1,1) = −3/5 = −p₂/p₃, C₃(1,3) = 8/5 = p₁³/p₃ (unique positive off-diagonal in any block), C₇(1,1) = −1/(p₂·p₃). Composite product det numerator = p₂²·p₃³·p₄² = 55125 (framework primes only; denominator uses shadow primes 11,13,17,19,23). Both even and odd composites share p₃³ factor. Odd eigenvalues 3 orders of magnitude below even (suppressed sector). | 6 identities (#363: bilateral annihilation theorem — DERIVED, EXACT. #364: C₃(1,1) = −p₂/p₃ — DERIVED, EXACT. #365: C₃(1,3) = p₁³/p₃ — DERIVED, EXACT, unique positive off-diagonal. #366: C₇(1,1) = −1/(p₂·p₃) — DERIVED, EXACT. #367: composite det numerator = p₂²·p₃³·p₄² — DERIVED, EXACT. #368: p=3 odd block complex eigenvalues — DERIVED, structural). |
| **C₀ Mechanism** | NB195 | C₀ mechanism characterization: full 210-branch JAX analysis of how cascade ODE produces C₀² = p₃p₄ = 35. κ-sensitivity: C₀(κ) extremely steep near κ₀ = 1/√P₄, 32% change in C₀ over 4% κ variation — value √35 requires κ₀ specifically (20-point scan). IC-diversity decomposition: at lepton g1 (ci=31), 99.7% of ⟨R₃²⟩ is inter-branch variance, 0.3% is mean². At g2 (ci=61), 55.1% mean² (driven) + 44.9% variance (residual IC). j₄ decomposition: quark g1 shows odd/even alternation (j₄=1,3,5 at ~94%), lepton g1 bell-shaped peaking at j₄=3,4. Effective decay rate γ₃/κ ≈ 0.893 (cf. λ(7)/p₄ = 6/7 = 0.857, 4.2% off). HONEST NULL: simple wrapping model (R₃ = 2πj₄·e^{−γt} wrapped to [−π,π]) fails at 43% — cascade cross-coupling from lower levels is ESSENTIAL. C₀ is genuinely dynamical: requires full ODE, κ = 1/√P₄, 210-branch IC diversity, and cascade cross-coupling. | System characterization, 0 new identities. #333 C₀(lepton)=√35 reconfirmed (0.07%). #334 C₀(quark)=21/√10 reconfirmed (0.51%). GD-2 (C₀ analytic derivation): OPEN. Mechanism understood qualitatively; no closed-form derivation. |
| **Geometric C₀** | NB196 | Floquet degeneracy theorem: linearized covering cascade is lower-triangular with constant diagonal −κ, giving 4-fold degenerate Floquet multiplier μ = e^{−κ} = 0.933. No mode splitting in linear regime → C₀ = 1 (all sectors identical). C₀ ≠ 1 is intrinsically NONLINEAR (sin() covering potential). Per-level IC decay hierarchy from full 210-branch integration: γ₀/κ = 0.41, γ₁/κ = 0.30, γ₂/κ = 0.11, γ₃/κ ≈ 0 — inner levels homogenize faster, outer IC diversity persists. Branch subset test: quark C₀ at R₃ invariant under inner-level conditioning (CV = 0.05% across 6 subsets of 35 branches) → effective covering degree = p₃p₄ = 35. Lepton C₀ at R₃ requires full P₄ = 210 branches (CV = 12.4%). Per-level sensitivity: quark CV converges to <1% only at R₂, R₃; lepton CV > 10% even at R₃. Chirality label a₃ (from p₂ = 3) acts as symmetry filter: a₃=1 (quark) cancels inner IC diversity, a₃=0 (lepton) preserves it. | System characterization, 0 new identities. GD-2: OPEN. Floquet degeneracy rules out linear/spectral derivation of C₀. Covering split identifies 35-group (quark) vs 210-group (lepton) distinction. |
